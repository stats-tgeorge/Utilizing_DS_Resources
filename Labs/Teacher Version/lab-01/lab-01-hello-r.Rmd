...
### Step 4
Next scroll all the way to the bottom and click generated as show in the image:
...
### Step 6
Paste that code in your lab RMarkdown under step 6. 
...
Now technically, we could be done but we don't want to have to repeat this every time we use RStudio. Thus next we are going to tell it to store your password. 

### Step 8
Now, copy the line from step 6 (with the GITHUB_PAT stuff). Run the step 8 chunk in your lab and paste in the new file that opens. Then click save (little floppy disk symbol at the top). See the image below. 
...
knitr::include_graphics("img/github-conn-6.PNG")
```

### Step 9
Click save (little floppy disk symbol at the top). See the image above. 
...
-   Go to RStudio Server and into the course workspace. Create a **New Project from Git Repo**. You will need to click on the down arrow next to the **New Project** button to see this option.
